...
int main(int argc, char *argv[]) {
    ...
    const char *output_filename = "1.mp4";
    ...
    avcodec_parameters_copy(output_stream->codecpar, codec_par);
    avio_open(&output_fmt_ctx->pb, output_filename, AVIO_FLAG_WRITE);
    int res = avformat_write_header(output_fmt_ctx, NULL);
    if(res != 0) {
    ...
    av_freep(&output_fmt_ctx->streams[0]);
